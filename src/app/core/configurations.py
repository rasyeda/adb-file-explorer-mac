...
def resource_filename(package: str, resource: str) -> str:
    """
    Minimal stand-in for pkg_resources.resource_filename (which newer
    setuptools releases have dropped). Since py2app bundles 'app' and
    'resources' as real on-disk folders rather than zip-imported packages,
    resolving via the imported package's own __file__ is reliable and adds
    no extra dependency.
    """
    module = __import__(package, fromlist=['__dummy__'])
    return os.path.join(os.path.dirname(os.path.abspath(module.__file__)), resource)
...
class Settings(metaclass=Singleton):
    ...
    # Packaged macOS apps may be read-only (signed bundles, translocated
    # copies, etc.), so settings live under Application Support rather than
    # next to the bundled source. Falls back to the bundled copy if that
    # directory can't be created for some reason.
    try:
        _support_dir = os.path.join(
            os.path.expanduser('~'), 'Library', 'Application Support', 'ADB File Explorer'
        )
        os.makedirs(_support_dir, exist_ok=True)
        filename = os.path.join(_support_dir, 'settings.json')
    except OSError:
